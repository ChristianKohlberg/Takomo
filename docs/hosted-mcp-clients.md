...
behaviors, linked tests, reported runs and environments — so an agent on stdio can run the
...
| Approve does nothing; the browser console says a form submission "violates the following Content Security Policy directive: form-action" | A takomo older than this fix. The consent page sent `form-action 'self'`, and a browser applies that to the 302 onward to the client's callback, not just to the form's own POST — so approving was blocked at the last hop. Fixed by naming the verified callback origin in the policy; upgrade the server. |
