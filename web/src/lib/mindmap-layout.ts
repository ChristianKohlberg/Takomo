...
export function childrenOf(nodes: MindmapNode[]): Map<string | null, MindmapNode[]> {
  const out = new Map<string | null, MindmapNode[]>()
 ...
 */
export function layout(nodes: MindmapNode[]): Layout {
 ...
 */
function centreAgainstChildren(id: string, placed: PlacedNode[], fallbackTop: number): number {
...
export function nodeAt(placed: PlacedNode[], world: Point): PlacedNode | null {
...
/** The curve from a parent's right edge to a child's left edge. */
 ...
 */
export function positionAfter(siblings: MindmapNode[], after: string): number | null {
