...
export function Workspace(props: Props) {
  ...
  return <div className="mx-auto max-w-5xl space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-2"><span className="text-muted-foreground text-sm">{lanes?.length ?? '—'} {lanes?.length === 1 ? t.laneSingular : t.title}</span>{canWrite && <Button onClick={() => setCreating(true)}>{t.newLane}</Button>}</div>
    ...
    {creating && <form className="bg-card space-y-3 rounded-xl border p-4" onSubmit={e => { e.preventDefault(); const form = new FormData(e.currentTarget); setBusy(true); void createLane(token, project, { title: String(form.get('title')).trim(), purpose: String(form.get('purpose')).trim() }).then(lane => { if (active.current) setSelected(lane.id) }).catch(fail).finally(() => { if (active.current) setBusy(false) }) }}>
      <label className="block text-sm">{t.laneTitle}<Input name="title" required maxLength={200} /></label><label className="block text-sm">{t.purpose}<Textarea placeholder={t.purposeExample} name="purpose" /></label><div className="flex gap-2"><Button disabled={busy}>{t.create}</Button><Button type="button" variant="ghost" onClick={() => setCreating(false)}>{t.cancel}</Button></div>
    </form>}
...
function LaneEditor({ lane, writable, busy, t, onSave }: { lane: Lane; writable: boolean; busy: boolean; t: Labels; onSave: (body: { title: string; purpose: string; context: string }) => Promise<boolean> }) {
  ...
  return <form className="space-y-3" onSubmit={e => { e.preventDefault(); void onSave(value).then(saved => { if (saved) setDirty(false) }) }}>
    ...
    <label className="block text-sm">{t.purpose}<Textarea placeholder={t.purposeExample} value={value.purpose} disabled={!writable || busy} onChange={e => { setDirty(true); setValue({ ...value, purpose: e.target.value }) }} /></label>
    <label className="block text-sm">{t.context}<Textarea placeholder={t.contextExample} value={value.context} disabled={!writable || busy} onChange={e => { setDirty(true); setValue({ ...value, context: e.target.value }) }} /></label>
