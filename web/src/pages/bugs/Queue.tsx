  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (detail && detail.ticket.project !== project) { setBug(null); update({ bug: '' }, true) } else setBug(detail)
      } catch (e) { if (active) { if (isAuthError(e)) onAuthError(); else setError(e instanceof Error ? e.message : String(e)) } }
    }
    void load()
    const timer = setInterval(() => void load(), 5000)
    return () => { active = false; controller.abort(); clearInterval(timer) }
  }, [token, project, view, severity, search, state, assignee, researchStatus, offset, selected, version, onAuthError, update])
  ...
  return <div className="space-y-4">
    ...
    <div className="grid min-w-0 gap-4 md:grid-cols-[minmax(0,1fr)_minmax(0,1.3fr)]">
      <section className="min-w-0 space-y-2" aria-label={t.title}>
        ...
        {page?.items.map(item => <button key={item.ticket.id} className={`block w-full min-w-0 rounded-xl border p-3 text-left hover:bg-muted ${selected === item.ticket.id ? 'border-primary bg-muted' : 'bg-card'}`} aria-pressed={selected === item.ticket.id} onClick={() => { setBug(item); update({ bug: item.ticket.id }) }}><span className="text-xs text-muted-foreground">{item.ticket.id}</span><span className="block break-words font-medium">{item.ticket.title}</span><span className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground"><span>{t.severity}: {label(item.severity)}</span><span>{t.triage}: {label(item.triage)}</span><span>{t.status}: {item.ticket.state}</span>{item.latest_job && <span>{t.researchStatus}: {label(item.latest_job.status)}</span>}</span></button>)}
