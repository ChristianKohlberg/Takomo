...
export function useProjectUpdates(token: string, project: string, refresh: () => Promise<unknown>) {
  ...
  useEffect(() => {
    ...
    const update = async () => {
      ...
      do {
        again = false
        try { await onRefresh() } catch { /* the owning page renders request errors */ }
      } while (again && !stopped)
      refreshing = false
    }
    const connect = async () => {
      try {
        ...
        socket.onmessage = () => { void update() }
        socket.onclose = () => { if (!stopped) timer = setTimeout(() => void connect(), 2000) }
      } catch { if (!stopped) timer = setTimeout(() => void connect(), 2000) }
    }
    void connect()
