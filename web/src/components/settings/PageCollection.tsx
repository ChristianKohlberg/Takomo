...
export function PageCollection({ lang, project = '' }: { lang: Locale; project?: string }) {
  ...
  return <section aria-labelledby="page-collection-title" className="space-y-3">
    <h2 id="page-collection-title" className="text-lg font-semibold">{de ? 'Ältere Arbeitsbereiche' : 'Older workspaces'}</h2>
    <p className="text-sm text-muted-foreground">{de ? 'Weiterhin verfügbar, wenn du sie brauchst. Inhalte und Berechtigungen bleiben erhalten.' : 'Available when you need them. Existing content and permissions are preserved.'}</p>
    <div className="divide-y">
      {pages.map(([path, label, description]) => <Link key={path} to={project ? `${path}?project=${encodeURIComponent(project)}` : path!} className="flex items-center gap-3 py-4 text-foreground hover:bg-muted focus-visible:outline-2 focus-visible:outline-primary">
