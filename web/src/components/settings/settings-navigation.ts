...
export type SettingsSection = ProjectSection | 'overview' | 'data' | 'access' | 'people' | 'projects' | 'library' | 'search' | 'github' | 'codex'
...
export function settingsSection(search: string): SettingsSection {
  ...
  return key && (isProjectSection(key) || ['overview', 'data', 'access', 'people', 'projects', 'library', 'search', 'github', 'codex'].includes(key))
...
export const settingsLabels = (lang: Locale): Record<SettingsSection, string> => lang === 'de' ? {
  ...
  overview: 'Deine Sitzung', data: 'Sicherung & Wartung', access: 'API-Tokens', people: 'Personen', projects: 'Projekte', library: 'Workflow-Bibliothek', search: 'Suche', github: 'GitHub', codex: 'KI-Verbindungen',
} : {
  general: 'General', writing: 'Writing instructions', documents: 'Document appearance', workflow: 'Workflow & timing',
  overview: 'Your session', data: 'Backup & maintenance', access: 'API tokens', people: 'People', projects: 'Projects', library: 'Workflow library', search: 'Search', github: 'GitHub', codex: 'AI connections',
}
