...
describe('document command menu', () => {
  ...
  it('resumes an unsent draft, action and scope through Open document conversation', async () => {
    render(<DocumentAgent {...props} />)
    const button = screen.getByRole('button', { name: 'Open document conversation' })
    command(button)
    ...
    command(button)
    fireEvent.click(await screen.findByRole('option', { name: 'Open document conversation' }))
    expect((await screen.findByRole('textbox') as HTMLTextAreaElement).value).toBe('My unfinished question')
    expect((screen.getByRole('radio', { name: 'Selected sections' }) as HTMLInputElement).checked).toBe(true)
