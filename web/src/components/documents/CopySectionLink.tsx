...
export function CopySectionLink({ href, locale }: { href: string; locale: Locale }) {
  ...
  return <span className="section-copy-link inline-flex min-w-0 items-center gap-2">
    <button type="button" className="inline-flex items-center gap-1 text-muted-foreground hover:text-foreground"
      aria-label={de ? 'Abschnittslink kopieren' : 'Copy section link'} title={de ? 'Abschnittslink kopieren' : 'Copy section link'}
    ...
    <span role="status" className="sr-only">{status === 'copied' ? (de ? 'Kopiert' : 'Copied') : ''}</span>
