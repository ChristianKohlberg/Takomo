...
import { threadsFor, type PaneDoc as PaneDocModel } from '@/lib/initiative-doc'
import type { Entry } from '@/lib/initiatives'
import { cn } from '@/lib/utils'

export interface PaneDocLabels extends MarginThreadLabels {
  unwritten: string
        ...
        return (
          <div key={i} className="contents">
            <div className="min-w-0 pt-1 pb-3">
              <p
                className={cn(
                  ...
                  p.uncited && 'decoration-destructive/70 underline decoration-dotted underline-offset-4',
                )}
              >
                {p.runs.map((run, j) =>
                  'text' in run ? (
            ...
            <div className="mb-3 flex min-w-0 flex-col gap-2 md:mb-0 md:pt-1">
              {i === 0 && inspector}
              {threads.map((t) => (
                <MarginThread key={t.entry.id} thread={t} labels={labels} />
              ))}
            </div>
          </div>
        )
