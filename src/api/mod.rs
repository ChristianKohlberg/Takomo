...
//
// `web/dist/` is committed for exactly this reason: `cargo build --release` must
// not require node. The CI gate rebuilds it and diffs, so the committed output
// cannot drift from `web/src/`.
//
