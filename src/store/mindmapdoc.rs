...
/// A section's prose fragment, for a reader.
///
...
pub fn section_prose(doc: &Doc, node: &str) -> ApiResult<XmlFragmentRef> {
    ...
    match entry.get(&txn, PROSE_KEY) {
        Some(Out::YXmlFragment(frag)) => Ok(frag),
        // A node that predates prose, read before `ensure_prose` has run. An
        // empty fragment is the honest answer: it has no prose yet.
        _ => {
            drop(txn);
            let mut txn = doc.transact_mut();
            let entry = node_map(&txn, &nodes_map, node)?;
            Ok(prose_of(&mut txn, &entry))
        }
    }
}
