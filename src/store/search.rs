...
c.fingerprint<>?2))",
            ...
            let pending: i64 = conn.query_row("SELECT count(*) FROM embedding_jobs WHERE map_id=?1 AND attempts<?2 AND lease_until<=?3", params![map, MAX_ATTEMPTS, now_ms()], |r| r.get(0))?;
