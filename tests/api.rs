...
async fn font_src_allows_the_embedded_display_face() {
    ...
    // The name is content-hashed, so find it where the browser does: in the
    // stylesheet's `url(...)`.
    let css = app
        .asset(common::spa_asset("app", ".css").trim_start_matches("/assets/"))
        .await;
    let at = css
        .find("geist-latin-wght-normal")
        .expect("the stylesheet should reference the embedded display face");
    let end = at + css[at..].find(".woff2").expect("the face is a woff2") + ".woff2".len();
    let woff2 = app
        .request(Method::GET, &format!("/assets/{}", &css[at..end]))
        .send()
        .await
        .unwrap();
    assert_eq!(
...
const KEYS_WITHOUT_A_COLUMN: [(&str, &str); 4] = [
    (
        "state_category",
    ...
    (
        "document_refs",
        "valid accepted ticket_document_links joined to current document sections",
    ),
    (
        "claim",
...
const KEYS_THE_SERVER_OWNS: [(&str, &str); 12] = [
    ("id", "generated by generate_ticket_id"),
    (
        "document_refs",
        "derived projection; changes use the document-links endpoints",
    ),
...
async fn deleting_an_initiative_takes_its_entries_and_leaves_the_work() {
    ...
    assert!(gone.get("detached_checks").is_none(), "{gone}");
...
/// The verification and environments surfaces are served, and their clients ship in the bundle.
///
...
async fn the_verification_surfaces_are_served_and_carry_their_clients() {
    ...
    for fragment in ["/behaviors", "/verification", "/environments"] {
        assert!(
...
///
/// A behavior stores a node ID, so every reader of the Tests view would
/// otherwise print `mn-…` where the section's name belongs, and linking a
/// behavior to a section would mean typing an opaque id. Resolved rather than copied on
