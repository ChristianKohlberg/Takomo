...
- `requires` entries (all must hold):
  ...
  - `guard:has_link:<key>` — the ticket must carry a non-empty `links.<key>`. This is the *evidence* family: it turns a state change from a claim into something a later reader can verify independently. `guard:has_link:commit` on the done edge is the intended use — a full commit SHA (or commit URL) stays checkable long after everyone involved has forgotten the ticket, and it is what release and deploy questions are derived from without any extra bookkeeping (`git tag --contains <sha>`, `git merge-base --is-ancestor <sha> <deployed-sha>`). The key is free-form, so a project can demand whatever its own proof is (`pr`, `run`, `env`). Failures are 409s naming the missing key with the `takomo_link` remedy — no ticket ids, because the subject is the ticket itself. Opt-in per project: the built-in `factory-default` does **not** include it.
