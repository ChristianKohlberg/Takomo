...
| scope     | grants |
|-----------|--------|
| `agent:run` | claim on-demand agent jobs and queued codebase extractions (`/v1/codebase-import-jobs/*`), heartbeat the owning attempt, and deliver its result, restricted to the token’s projects. Grants no specification, ticket, test or general read/write access. |
...
**Off by default.** The endpoints exist only when `TAKOMO_PUBLIC_URL` names the public origin the server is reached at (`https://takomo.example.com` — no path, no trailing slash; plain `http` is accepted only on loopback). Unset, they answer `404 temporarily_unavailable` naming the variable, and `/mcp` sends no `WWW-Authenticate` header. The value is checked at startup, not on first use, because a client compares the issuer and the resource identifier byte-for-byte against what it fetched and what the user typed — a stray path is not cosmetic, it is a connection that fails inside someone else's product.

**A value that fails that check turns OAuth off and says so on the startup line; it does not stop the server** (takomo-z919). This variable predates OAuth and still has a second, deliberately tolerant reader — the absolute links in ask-a-human notifications, which only ever needed a non-empty prefix. Someone who set it long ago for readable links, with a path prefix or a plain-`http` tailnet host, must not lose their instance to an upgrade that added a stricter reader of the same string; they never asked for OAuth. The bind guard below *does* refuse to boot, and the asymmetry is intended: that one prevents an unencrypted service reaching a network, whereas the whole cost here is that hosted clients cannot attach — which the `/oauth/*` routes report per request anyway.
